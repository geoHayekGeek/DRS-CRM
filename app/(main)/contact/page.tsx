  ...
  return (
    <div className="container mx-auto px-4 py-16 space-y-16">
      ...
      <div className="text-center space-y-4 animate-fade-in-up">
        ...
        <p className="text-black-500 font-bold uppercase tracking-widest text-sm md:text-base">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
        ...
        <div className="bg-white p-8 md:p-10 rounded-3xl shadow-2xl border border-gray-100 animate-slide-in-left">
          <h2 className="text-3xl font-black uppercase italic mb-8 text-black-900">
          ...
          <form className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-black-500">First Name</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-black-500">Last Name</label>
                <input 
                  type="text" 
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-all font-medium"
                  placeholder="Surname"
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-black-500">Email Address</label>
              <input 
                type="email" 
                className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-all font-medium"
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-black-500">Subject</label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-black-500">Message</label>
              <textarea 
                rows={4}
        ...
        <div className="space-y-8 animate-slide-in-right">
          ...
          <div className="bg-black text-white p-8 rounded-3xl shadow-xl relative overflow-hidden group">
            ...
            <div className="flex gap-4">
              <SocialBtn href="https://www.instagram.com/drslanelb" icon={<FaInstagram />} label="Instagram" />
              <SocialBtn href="https://www.tiktok.com/@drslanelb" icon={<FaTiktok />} label="TikTok" />
              <SocialBtn href="https://youtube.com/@drslanelb" icon={<FaYoutube />} label="YouTube" />
            </div>
          </div>
...
const InfoCard = ({ icon, title, line1, line2 }: { icon: React.ReactNode, title: string, line1: string, line2: string }) => (
  <div className="flex items-start gap-4 p-6 bg-white rounded-2xl border border-gray-100 hover:border-red-200 hover:shadow-lg transition-all duration-300 group">
    <div className="w-12 h-12 bg-gray-50 text-black-400 rounded-full flex items-center justify-center text-xl group-hover:bg-red-600 group-hover:text-white transition-colors duration-300 shrink-0">
      {icon}
    </div>
    <div>
      <h3 className="text-lg font-bold uppercase italic text-black-900 mb-1">{title}</h3>
      <p className="text-black-600 text-sm font-medium">{line1}</p>
      <p className="text-black-400 text-xs font-medium uppercase tracking-wide mt-1">{line2}</p>
    </div>
  </div>
);

const SocialBtn = ({ icon, href, label }: { icon: React.ReactNode, href: string, label: string }) => (
  <Link href={href} target="_blank" rel="noopener noreferrer" className="w-14 h-14 rounded-xl bg-gray-800 flex items-center justify-center text-black-400 text-2xl hover:bg-white hover:text-black transition-all duration-300 hover:scale-110 hover:shadow-[0_0_20px_rgba(255,255,255,0.3)]" aria-label={label}>
    {icon}
  </Link>
);
